    ...
    if (isV2 && job.export_type === 'pdf') {
      ...
      const layoutDocument = await generateLayoutV2({
        ...
        selection: {
          ...
          bannerHeroLabelPosition: (config?.bannerHeroLabelPosition === 'top' || config?.bannerHeroLabelPosition === 'none') ? config.bannerHeroLabelPosition : (config?.bannerHeroLabelPosition === 'bottom' ? 'bottom' : undefined),
          bannerHeroLabelColor: config?.bannerHeroLabelColor === 'dark' ? 'dark' : (config?.bannerHeroLabelColor === 'light' ? 'light' : undefined),
