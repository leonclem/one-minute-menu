...
export function insertFillerTilesIntoLayout(
  ...
  // Process each page
  const pagesWithFillers = layout.pages.map(page => 
    insertFillerTilesIntoPage(page, template)
  )
...
/**
 * Insert filler tiles into a single page
 * 
...
function insertFillerTilesIntoPage(
  ...
  // Find empty cells
  const emptyCells = findEmptyCells(occupancyGrid, baseCols, baseRows)
  
  // Generate filler tiles for empty cells
  const fillerTiles = emptyCells.map((cell, index) => 
    createSpacerTile(cell.col, cell.row, index)
  )
...
/**
 * Create a SPACER tile for an empty cell
 * 
