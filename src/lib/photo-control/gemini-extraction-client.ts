...
Use the complete lighting key set: bright-clean, soft-natural, golden-hour, dark-moody, or bold-sunlight. Classify scene_setup.angle as top-down only when the camera looks straight down and the plate reads as a circle or oval from above; typical restaurant three-quarter food photos are 45-degree. Describe the observed backdrop and tabletop surface with material and six-digit hex colour when visible. Set backdrop_visible and surface_visible from the photograph; do not invent a value when the evidence is unavailable. Include a concise prose description of the complete composition. Return empty arrays for garnishes and sides when none are present. Values must describe what is observed, not a requested edit. You may optionally include spatial_inventory with only coarse center or region hints for visible composition elements; never return masks, pixel coordinates, or provider IDs.`
