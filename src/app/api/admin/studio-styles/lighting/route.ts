/**
 * Admin — lighting style library list + create.
 *
 * GET  /api/admin/studio-styles/lighting
 * POST /api/admin/studio-styles/lighting
 */
...
export async function GET() {
  ...
  try {
    const styles = await listAllLightingStyles()
    return NextResponse.json({ styles })
  } catch (error) {
    logger.error('Failed to list lighting styles', { error })
...
export async function POST(request: NextRequest) {
  ...
  try {
    const style = await createLightingStyle(parsed.data)
    ...
    logger.error('Failed to create lighting style', { error })
