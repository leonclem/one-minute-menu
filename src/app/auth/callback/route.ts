    ...
    if (!sessionError) {
      try {
        ...
        if (user) {
          ...
          // First verified login (magic-link click), not Auth created_at —
          // the user row is created when the email is submitted, often minutes
          // before they open the link.
          if (isFirstVerifiedLogin(profile?.lastLoginAt)) {
