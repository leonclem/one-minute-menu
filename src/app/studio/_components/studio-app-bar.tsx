...
export function StudioAppBar({ showCredits, userEmail, isGuest = false }: StudioAppBarProps) {
    ...
    <header className="sticky top-0 z-30 border-b border-white/[0.07] bg-[#0c1416]/90 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6">
        ...
        <div className="flex shrink-0 items-center gap-3 sm:gap-4">
          ...
          {isGuest ? (
            <Link href="/auth/signin" className="text-sm font-semibold text-white/55 hover:text-white">
              Sign in
            </Link>
          ) : (
            <>
