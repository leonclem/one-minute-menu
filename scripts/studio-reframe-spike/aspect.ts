...
/** Nearest Flash aspect ratio for a pixel size. Used to keep zoom-out in the source shape. */
...
export function aspectRatioHonoured(
  width: number,
  height: number,
  requested: FlashAspectRatio,
  tolerance = 0.1,
): boolean {
  const actual = width / height
  const expected = ratioValue(requested)
