...
const resultPath = join(resultDirectory, 'move-nb2-batch-03.json')
...
const downloadApprovedBatch = process.env.DOWNLOAD_APPROVED_NB2_MOVE_BATCH_03 === 'true' ? it : it.skip

downloadApprovedBatch('downloads and verifies all returned Move/NB2 batch 03 artifacts for internal review only', async () => {
  ...
  expect(failedRecords).toEqual([
    expect.objectContaining({
      caseId: 'move-08-roast-beef-knife',
      variant: 'B',
