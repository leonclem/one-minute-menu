...
// Run with: npx ts-node test-prompt-construction.ts

import { 
  getPromptConstructionService, 
  createBasicPrompt, 
  validateMenuItemForImageGeneration 
} from './src/lib/prompt-construction.js'
import type { MenuItem } from './src/types/index.js'
...
function testPromptConstruction() {
  try {
    ...
    const styles = ['modern', 'rustic', 'elegant', 'casual'] as const
